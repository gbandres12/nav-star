...
export default async function Vender({ searchParams }: PageProps<"/admin/vender">) {
  ...
  // Sem filtro de cidade: lista todos os trechos com saída na data
  const pares = cidades.flatMap((o) => cidades.filter((d) => d.id !== o.id).map((d) => [o.id, d.id] as const));
  const resultados = pares
    .filter(([o, d]) => (!origem || o === origem) && (!destino || d === destino))
    .flatMap(([o, d]) => buscarViagens(o, d, data))
            ...
            <tbody>
              {resultados.map((r) => (
                <tr key={`${r.viagem.id}-${r.origemOrdem}-${r.destinoOrdem}`}>
                  ...
                  <td>{duration(r.duracaoMin)}</td>
                  <td className="text-right font-bold tabular-nums">{money(r.valor)}</td>
                  <td className="text-right tabular-nums">{money(r.taxa)}</td>
                  <td>
                    <span className={`inline-flex min-w-9 justify-center rounded-full px-2 py-0.5 text-xs font-bold ${r.livres < 15 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>{r.livres}</span>
                  </td>
                  <td>
